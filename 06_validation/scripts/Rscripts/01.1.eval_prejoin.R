...
get_vc <-  function(p){ # each ploidy we ran so far
  ...
  ts <- data.frame()
  for (i in c("SRR1997411",  "SRR3189741",  "SRR3189742",  "SRR3189743")){ # each sample 
      t <- read.table(gzfile(paste0(dir, "VC_out/", i, "/", i, ".ploidy_", p ,".g.vcf.gz" )))  
  ...
  colnames(t)<- c("CHROM",	"position",	  "ID", 	"REF",  	"ALT",
                   "QUAL", "FILTER",	"INFO",	"FORMAT",	"data", "sample", "ploidy", "eval_sample")
  
  t<- t[ , c(2,4,5,6,11,12,13,9,10,8) ]
  return(t)
}
...
get_vc_filtering <-  function(p, version="v1") { # each ploidy we ran so far
  ts <- data.frame()
  for (i in c("SRR1997411",  "SRR3189741",  "SRR3189742",  "SRR3189743")){ # each sample 
      t <- read.table(gzfile(paste0(dir, "VC_out/", i, "/", i, ".ploidy_", p ,"_filtering.g.vcf.gz" )))    
  ...
  print(paste0("Done with prepering all samples for ploidy ", p))
  colnames(ts)<- c("CHROM",	"position",	  "ID", 	"REF",  	"ALT",   "QUAL", "FILTER",	"INFO",	"FORMAT",	"data", "sample", "ploidy", "eval_sample")
...
classify_variant_sample <- function(position, sample, df, true_variants) {
  ...
  if (!position %in% true_variants$position) {
    ...
    if (df[df$position == position & df$sample == sample, "GT"] == ".") {
      ...
      if (length(alleles) == 1) {
        ifelse(alleles == ref_allele, return("FN_ref_homozygous"), return("TP"))
