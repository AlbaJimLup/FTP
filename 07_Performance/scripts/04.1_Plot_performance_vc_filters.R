...
palette<- c("#ffaa2c", "#FF7641", "#9a1142", "#e767c5", "#8a76bc", "darkblue", "royalblue", "#56ae6c", "#184e3c")
palette<- c("#184e3c", "#56ae6c", "royalblue", "darkblue", "#8a76bc", "#e767c5", "#9a1142", "#FF7641", "#ffaa2c")
...
png(file=paste0(dirI, "Precision_vs_Recall_vc_filters.png"), res=350, width=1000, height=800);
...
ggplot(best, aes(x = recall, y = precision, color = factor(set), shape = factor(eval_level))) +
  ...
  guides(shape = guide_legend(override.aes = list(size=1.5)))+
  # geom_point(shape=3,size=2, alpha=0.6) +
  coord_cartesian(xlim =c(0.7, 0.95), ylim = c(0.925, 0.94))+
  theme(legend.key.size = unit(0, 'lines'), 
        legend.key.height = unit(3, 'mm'), #change legend key height
        legend.key.width = unit(0, 'cm'),
        legend.spacing.x = unit(1, 'mm'),
...
ggplot(best, aes(x=F1_score, y = factor(set) , fill = factor(eval_level))) +
  ...
  labs( y="Ploidy", fill="")+
  theme_minimal()+
  theme( axis.text.x = element_text(size=7,vjust=1, hjust=1),  axis.title = element_text(size=10),
         legend.key.width = unit(2, 'mm'),legend.margin=margin(-15,-4,-2,-22),    axis.text.y = element_text(size=10))
dev.off();
