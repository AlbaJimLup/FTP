...
# root<- "~/Documents/mn5/Projects/ribosomal_RNAs/Alba/"
...
true_variants <- true_variants[true_variants$class=="callable",]

not_FN<- c()
### Get bed callable blacklisted regions  ######
bed <- read.table(paste0(root, "/data/pre-rRNA_47S.included.bed"))

get_vc_filtering <-  function(p, version="v4") { # each ploidy we ran so far
  ...
  for (i in 1:100){ # each sample 
    for (region in c("5_ETS", "18S", "ITS1", "5.8S", "ITS2",  "28S", "3_ETS")){
      ...
      t <- t[t$V5 != "<NON_REF>", ] #This is in case we don't do filter
      ts <-  rbind(ts, t) # non-called but possible non-reference alleles
    }
  }
...
#Remove alleles in ALT that are not in GT
remove_non_called_alleles <- function(df){
  df <- df[,c(1:8,12)]
  alternatives <- strsplit(df$ALT, ",")
  genotypes <- strsplit(df$GT, "/")
  df$ALT <- sapply(1:nrow(df), function(x) paste0(alternatives[[x]][alternatives[[x]] %in% genotypes[[x]]], collapse=","))
  return(df)
}
...
# VC evaluation per allele
classify_variant_allele <- function(df, true_variants) {
  for(sample in unique(df$sample)){
    fns <- true_variants[true_variants$sample==sample,] #This data frame will eventually contain the fns
    fns$row <- rownames(fns)
    sample_pos <- unique(df$position[df$sample == sample])
    for(position in sample_pos){
      position<- as.numeric(position)
      alleles <- strsplit(df[df$position==position & df$sample==sample, "ALT"], ",")[[1]]
      classifications <- c() #For multiallelic positions
      for(allele in alleles){ #Iterating over the alternative alleles
        if (!position %in% fns$position[fns$sample == sample]) { #If the position is not true
          classification <- "FP"
          #We know GATK might call a shifted variant. So it is really a TP but is shifted one position, Alba has some examples in classify_variant_sample and Fairlie must have an automatic code to fix this
        } else{ #If the position is true
          if(allele %in% fns[fns$position==position & fns$sample==sample, "alt"]){
            classification <- "TP"
            fns <- fns[fns$row!=fns[fns$position==position & fns$sample==sample & fns$alt==allele,"row"],] #We remove the row that is a TP from the data frame that at the end of the iteration will contain FNs
          } else{
            classification <- "FP"
          }
        }
      ...
      #We are done iterating over alleles, so we will append to df the results:
      df$eval_sample[df$position==position & df$sample==sample] <- paste0(classifications, collapse=",")
    }
    #We now have all alleles from df for the given sample filled. Now we will add the FNs to df
    fns_formatted <- fns[,c(5,6,7,8)]
    fns_formatted <- cbind(fns_formatted, "FN", sample, ploidy, "FN", "FN", "callable")
    fns_formatted <- fns_formatted[,c(2,3,4,5,6,7,8,1,9,10)]
    colnames(fns_formatted) <- colnames(df)
    fns_formatted$region <- gsub("'", "_", fns_formatted$region)
    
    df <- rbind(df, fns_formatted)
  }
  return(df)
}
...
#Remove alleles in d$ALT that are not called. We also clean the dataframe by removing some columns
d <- remove_non_called_alleles(d)

# Classify callable and blacklisted positions. But all of them are callable of course
d <-  get_black(d)

## Get evaluation sample level for GATK ploidies
d <- classify_variant_allele(d, true_variants=true_variants)



if(step=="filtering"){
  write.table(d, paste0(dirf, "Allele_VCeval_prejoin_p", ploidy, "_", step,"_", version,".tbl"), sep="\t", row.names=F)
}else{
  write.table(d, paste0(dirf, "Allele_VCeval_prejoin_p", ploidy, "_", step, ".tbl"), sep="\t", row.names=F)
}
print("Done!")
print("------------------------------------")
