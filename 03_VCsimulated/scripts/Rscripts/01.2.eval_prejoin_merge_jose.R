...
if(step=="filtering"){
  for (ploidy in ploydies){
      print( paste0(dirf,  "Allele_VCeval_prejoin_p", ploidy, "_", step,"_", version,".tbl"))
      d<- read.table(paste0(dirf, "Allele_VCeval_prejoin_p", ploidy, "_", step, "_", version,".tbl"), sep="\t", header = T)
      # Save to have a single output file with all ploidies 
      all<- rbind(all, d)
  }
  write.table(all, paste0(dirf, "Allele_All_VCeval_prejoin_", step, "_", version, ".tbl"), sep="\t")
}else{
  for (ploidy in ploydies){
      print( paste0(dirf,  "Allele_VCeval_prejoin_p", ploidy, "_", step, ".tbl"))
      d<- read.table(paste0(dirf, "Allele_VCeval_prejoin_p", ploidy, "_", step, ".tbl"), sep="\t", header = T)
      # Save to have a single output file with all ploidies 
      all<- rbind(all, d)
  }
  write.table(all, paste0(dirf, "Allele_All_VCeval_prejoin_", step, ".tbl"), sep="\t")
}
