...
if(step=="joingen"){
  dirI<- paste0(dir, "IMAGES/02_POSTJOIN/", set, "/joingen/")
  dfs <- read.table( paste0(dirf,"VCeval_postjoin_joingen.tbl"), sep="\t")
  
}else{
  dirI<- paste0(dir, "IMAGES/02_POSTJOIN/", set, "/filtering_", version, "/")
...
write.table(positions_t, paste0(dirf, "Positions_per_evaluation", version,".tbl"))
...
write.table(positions_FPFN, paste0(dirf, "FPFN_Positions_per_evaluation",version ,".tbl"))
...
ggsave(filename = paste0(dirI, "00_VCevaluation_callable_ploidy_joingen.png"), plot = plot1, dpi = 500, width = 2500/500, height = 1500/500)
