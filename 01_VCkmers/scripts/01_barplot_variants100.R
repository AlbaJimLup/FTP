...
library(dplyr)
# Get table
s <- read_csv(paste0(dir, "CHM13_variants.msa_vs_diploid_GATK - 100x.csv"), show_col_types = FALSE)
s <-  as.data.frame(s)
##########----------------------------- DATA prep ----------------------------#############
...
write.table(diverging, paste0(dir, "x100diverging_positions_table.tbl"), sep = "\t", quote = FALSE, row.names = FALSE)
...
counts_ev <- counts_ev %>%
  filter(counts != 0) 
# Export counts table:
write.table(counts_ev, paste0(dir,  "x100stats_evaluationGATKvsMSA.tbl"), sep = ",", quote = FALSE, row.names = FALSE)

png(file=paste0(dirI,"x100GATKvsMSAalleles.png"), res=300, width=2300, height=1300);
ggplot(counts_ev, aes(x = fct_reorder(class, freqs), y =freqs)) +
  geom_bar(stat = "identity", aes(fill = evaluation))+
  geom_text(aes(label = c("39","1","1","60","43","5","10","12")), 
            position = position_stack(vjust = 0.3), size = 3.5,fontface = "bold") +
  theme_minimal()+
  labs(title ="GATK compared to MSA Allele Variants",
       subtitle = "Kmers of depth 100x",
       y="Frequency")+
